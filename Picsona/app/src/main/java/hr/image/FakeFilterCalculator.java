...
public class FakeFilterCalculator {
    ...
    public Bitmap calculateOverlay(float gender, float anger, float sadness, float happiness, int doubleemojicount){
        ...
        if (numAngEmojis > 0 && anger >= 0.25f) {
            overlayGenerator.prepareEmojis(distinctEmojis, OverlayGenerator.EmojiType.Angry);
            bitmap = overlayGenerator.reCreateOverlayWithMoreEmojis(numAngEmojis);
        }
        if (numHapEmojis > 0 && happiness >= 0.25f) {
            overlayGenerator.prepareEmojis(distinctEmojis, OverlayGenerator.EmojiType.Happy);
            bitmap = overlayGenerator.reCreateOverlayWithMoreEmojis(numHapEmojis);
        }
        if (numSadEmojis > 0 && sadness >= 0.25f) {
            overlayGenerator.prepareEmojis(distinctEmojis, OverlayGenerator.EmojiType.Sad);
            bitmap = overlayGenerator.reCreateOverlayWithMoreEmojis(numSadEmojis);
        }
        return bitmap;
    }
    ...
    public Bitmap calculateOverlay(float gender, float anger, float sadness, float happiness){
        ...
        if(anger > 0.5f){
            overlayGenerator.prepareEmojis(distinctEmojis, OverlayGenerator.EmojiType.Angry);
            bitmap = overlayGenerator.reCreateOverlayWithMoreEmojis((int) Math.ceil(anger / 0.50f));
        }
        if(happiness > 0.5f){
            overlayGenerator.prepareEmojis(distinctEmojis, OverlayGenerator.EmojiType.Happy);
            bitmap = overlayGenerator.reCreateOverlayWithMoreEmojis((int)Math.ceil(happiness/0.50f));
        }
        if(sadness > 0.5f){
            overlayGenerator.prepareEmojis(distinctEmojis, OverlayGenerator.EmojiType.Sad);
            bitmap = overlayGenerator.reCreateOverlayWithMoreEmojis((int)Math.ceil(sadness/0.50f));
        }
